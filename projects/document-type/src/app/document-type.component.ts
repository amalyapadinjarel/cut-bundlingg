import { Component } from '@angular/core';
import { DocumentTypeSharedService } from './_service/document-type-shared.service';
import { DocumentTypeService } from './_service/document-type.service';
import { DocumentService } from 'app/shared/services';
import { Router } from '@angular/router';
import { AlertUtilities } from 'app/shared/utils';
import {Location} from '@angular/common';
import { Subscription } from 'rxjs';
import {SubSink} from 'subsink';
import { TnzInputService } from 'app/shared/tnz-input/_service/tnz-input.service';
@Component({

  templateUrl: './document-type.component.html',
  styleUrls: ['./document-type.component.scss']
})
export class DocumentTypeComponent {
  subs = new SubSink();
 constructor( 
  public _shared: DocumentTypeSharedService,
  public _service: DocumentTypeService,
  private docService: DocumentService,
  private router: Router,
  
  private alertutils: AlertUtilities,
  private location: Location
  
  ) {
    this._shared.init(); 
  }
  
  ngOnDestroy() {
    this._shared.clear();
   // this.subs.unsubscribe();
  }
 newDocument() {
  this.docService.checkAppPermission(this._shared.taskFlowName, 'create')
  .then(data => {
        this.router.navigateByUrl('/document-type/create').then(done => {
          this._shared.editMode = true;
         this._shared.initLocalCache();
         
        });
      }).catch(err => {
        this.alertutils.showAlerts(err)
      })
  }
 save(exit = false) {
  this._service.save(exit)
  //let isCreate = this._shared.id == 0;
   /*this._service.save(exit).then((flag) => {
   if (isCreate && !exit) {
      this.router.navigateByUrl('/document-type/' + this._shared.id + '/edit')
    }
    if (flag && exit) {
    
      this.cancelEdit();
    }
  })*/
 }
 editDcument(){
  this.docService.checkAppPermission(this._shared.taskFlowName, 'create')
  .then(data => {
      this.location.go('document-type/' + this._shared.id + '/edit')
      this._shared.editMode = true;
      this._shared.initLocalCache()

  })
  .catch(err => {

      this.alertutils.showAlerts(err)
  })
 } 
 cancelEdit(fromSave=false){
  // if(!fromSave )
  this._service.setStatusData();
  
  if (this._shared.id > 0) {
    this.location.go('/document-type/' + this._shared.id);
    this._shared.editMode = false;
   
    this._shared.initLocalCache();
   this._shared.resetLines();
   
} else {
    this.router.navigateByUrl('/document-type/list').then(done => {
        this._shared.editMode = false;
      
        this._shared.initLocalCache();
    });
}
 } 
} 