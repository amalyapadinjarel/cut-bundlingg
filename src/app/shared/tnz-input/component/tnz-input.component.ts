...
export class TnzInputComponent implements OnChanges, OnDestroy {
        ...
        if (!this.loading) {
            if (this.lovConfig) {
                if (this.lovConfig.returnKey) {
                    this.returnKey = this.lovConfig.returnKey;
                }
                if (this.lovConfig.displayKey) {
                    this.displayKey = this.lovConfig.displayKey;
                }
            }
    ...
    onValueChanged(data) {
        ...
        if (multiple && multiple.length) {
            this.multipleSelected.emit({
                key: this._key,
                displayKey: this.displayKey,
                setters: this.setters,
                lovSetters: this.lovSetters,
                lovConfig: this.lovConfig,
                models: multiple,
                path: this.fullPath
            });
        }
    ...
    loadSelectOptions() {
        if (this.selectOptions.length == 0 && this.lovConfig.url) {
            ...
            this._service.getSelectOptions(this.app, url, this.lovConfig.dataHeader).then(res => {
                if (res.length && this.lovFilter) {
                    this.selectOptions = res.filter(this.lovFilter);
                } else {
    ...
    getLabelFromSelectOptions(value) {
        if (this.selectOptions && this.returnKey && this.displayKey) {
            let option = this.selectOptions.find((op) => {
                return op[this.returnKey] == value
            })
